...
# Create the main dictionary for the patient
patient_data = {
    "name": "John Doe",
    "CBC_blood_panel_results": {
        (datetime.now() - timedelta(days=30)).strftime("%Y-%m-%d"): generate_cbc_panel(),
        (datetime.now() - timedelta(days=64)).strftime("%Y-%m-%d"): generate_cbc_panel()
    }
}
...
with open('src/patient_data.json', 'w') as json_file:
